...
module sva #(
  ...
  localparam logic [ADDR_W:0] DEPTH_COUNT_W        = DEPTH;
  localparam logic [ADDR_W:0] ALMOST_FULL_THRESH_W = ALMOST_FULL_THRESH;
  localparam logic [ADDR_W:0] ALMOST_EMPTY_THRESH_W = ALMOST_EMPTY_THRESH;
  localparam logic [ADDR_W:0] PTR_WRAP_VALUE_W     = DEPTH - 1;
  ...
  a_almost_full_iff_threshold : assert property (
    almost_full === ((count >= ALMOST_FULL_THRESH_W) && !full)
  ...
  a_almost_empty_iff_threshold : assert property (
    almost_empty === ((count <= ALMOST_EMPTY_THRESH_W) && !empty)
  ...
  // In this RTL, almost_full is masked off when full is asserted.
  a_full_implies_almost_full : assert property (
    full |-> !almost_full
  ) else $error("G2: full asserted while almost_full is also asserted");

  // In this RTL, almost_empty is masked off when empty is asserted.
  a_empty_implies_almost_empty : assert property (
    empty |-> !almost_empty
  ) else $error("G2: empty asserted while almost_empty is also asserted");
  ...
  a_wr_ptr_advance : assert property (
    write_fire |=>
      (wr_ptr === (($past(wr_ptr) == PTR_WRAP_VALUE_W)
                   ? '0
                   : $past(wr_ptr) + 1'b1))
  ...
  // Pointer must stay known after reset.
  a_wr_ptr_range : assert property (
    !$isunknown(wr_ptr)
  ) else $error("G3: wr_ptr is X");
  ...
  a_rd_ptr_advance : assert property (
    read_fire |=>
      (rd_ptr === (($past(rd_ptr) == PTR_WRAP_VALUE_W)
                   ? '0
                   : $past(rd_ptr) + 1'b1))
